...
class Reranker:
    ...
    def __init__(
        self,
        model_name: str = "cross-encoder/bge-reranker-v2-m3",
    ...
    async def rerank(
        ...
        def _sync_predict() -> list[float]:
            ...
            pairs = [(query, str(getattr(d, "content", ""))) for d in documents]
        ...
        for doc, sc in ranked[:top_k]:
            new_doc = doc.model_copy(deep=True) if hasattr(doc, "model_copy") else doc
            if hasattr(new_doc, "score"):
                new_doc.score = float(sc)
            metadata = getattr(new_doc, "metadata", None)
            if isinstance(metadata, dict):
                new_doc.metadata = {**metadata, "rerank_score": float(sc)}
            out.append(new_doc)
        return out
